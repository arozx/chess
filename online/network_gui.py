...
class NetworkedChessBoardUI(QWidget):
    ...
    def init_ui(self):
        ...
        )  # Add material count label to layout
    ...
    def move_piece(
        ...
        if self.chess_board.move_piece(source_row, source_col, target_row, target_col):
            ...
            print(
                f"Moved from ({source_row}, {source_col}) to ({target_row}, {target_col})"
            )
            self.move_count_label.setText(f"Move count: {self.chess_board.move_count}")

            self.player_to_move_label.setText(
                f"{self.chess_board.player_turn[0:1].upper()}{self.chess_board.player_turn[1::]} to move"
            )
            if captured_piece:
